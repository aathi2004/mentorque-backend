...
export async function scheduleMeeting(req, res, next) {
  try {
    ...
    // Mark the originating call request (if any) as booked, linking it to this meeting.
    const { callRequestId } = req.body;
    if (callRequestId) {
      await prisma.callRequest
        .update({
          where: { id: callRequestId },
          data: { status: "BOOKED", meetingId: meeting.id },
        })
        .catch((err) => {
          // Don't fail meeting creation if the call-request link-up fails (e.g. stale id).
          console.error("[scheduleMeeting] Failed to link callRequest", callRequestId, err?.message || err);
        });
    }
